# Documentation

**→ See [Documentation Index (INDEX.md)](INDEX.md) for complete navigation**

This directory contains all documentation for the Precision Medicine MCP system, organized by audience and topic.

---

## Quick Start

**New to the system?**
- [Installation Guide](getting-started/installation.md) - Complete setup (Quick Start: 5 minutes)
- [Executive Summary](EXECUTIVE_SUMMARY.md) - High-level overview for decision-makers
- [Why MCP for Healthcare?](WHY_MCP_FOR_HEALTHCARE.md) - Understand the architecture

**Find your guide:**
- 🔬 [For Researchers](for-researchers/README.md)
- 💻 [For Developers](for-developers/README.md)
- 🏥 [For Hospitals](for-hospitals/README.md)
- 🎓 [For Educators](for-educators/README.md)
- 👥 [For Patients](for-patients/README.md)
- 💰 [For Funders](for-funders/README.md)

---

## Complete Documentation

For comprehensive navigation including:
- Architecture guides
- Deployment documentation
- Testing strategies
- Compliance guides
- Clinical workflows
- And more...

**→ Visit the [Documentation Index (INDEX.md)](INDEX.md)**

---

**Last Updated:** 2026-01-19
